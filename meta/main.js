...
function processCommits(data) {
    return d3
        ...
        .map(([commit, lines]) => {
            ...
            let ret = {
                id: commit,
                url: 'https://github.com/chillingandcoding/Portfolio/commit/' + commit,
                author,
                date,
                time,
                timezone,
...
function renderCommitInfo(data, commits) {
    ...
    const totalFiles = d3.rollups(data, (d) => d.length, (D) => D.file);
    ...
    const fileLength = d3.max(totalFiles, d => d[1]);
    dl.append('dt').text('LONGEST FILE LENGTH');
    dl.append('dd').text(fileLength);
